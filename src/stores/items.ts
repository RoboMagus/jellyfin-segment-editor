...
import { BaseItemDto, BaseItemKind, VirtualFolderInfo } from '@jellyfin/sdk/lib/generated-client'
...
export const useItemsStore = defineStore('items', () => {
  ...
  const getItemCollections = async () => {
    ...
    if (coll.length)
      selectedCol.value = coll[0].ItemId as string
  }
  ...
  const collectionToType = computed(() => {
    const col = collections.value.find((col: VirtualFolderInfo) => col.ItemId == selectedCol.value)
  ...
  const isMixedType = (type: BaseItemKind) : boolean => {
    return collectionToType.value == BaseItemKind.CollectionFolder && (type == BaseItemKind.Movie
      || type == BaseItemKind.Series || type == BaseItemKind.Season || type == BaseItemKind.Episode)
  }

  // apply items filter
  const filteredItems = computed(() => localItems.value.filter((item) => (collectionToType.value == item.Type || isMixedType(item.Type as BaseItemKind)) && item.Name?.toLowerCase().includes(filterName.value.toLowerCase())))
