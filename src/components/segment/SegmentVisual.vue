...
const getStyle = computed(() => {
  return {
    left: Math.round(props.parentWidth / durationSecs * (props.segment.StartTicks ?? 0)) + 'px',
    width: Math.max(8, Math.round(props.parentWidth / durationSecs * ((props.segment.EndTicks ?? 0) - (props.segment.StartTicks ?? 0)))) + 'px',
  }
});
