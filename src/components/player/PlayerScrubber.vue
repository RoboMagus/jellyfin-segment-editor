<template>
  <div
    ref="scrubber"
    class="relative scrubber-container rounded-borders non-selectable bg-black"
  >
    <div
      class="relative-position overflow-hidden full-width rounded-borders cursor-pointer"
      @mousedown="scrubbing = true"
      style="height: 0.7rem"
    >
      <div
        class="fit absolute bg-deep-purple-3 rounded-borders opacity-40"
        :style="{ transform: `translateX(${(secondary / max) * 100 - 100}%)` }"
      />
      <div
        class="fit relative-position bg-deep-purple rounded-borders"
    ...
    <div class="absolute scrubber-overlay">
      <slot
        ...
        :position="`${Math.max(0, Math.min(elementX, elementWidth)) - 40}px`"
      />
    </div>
  </div>
</template>
