...
export function useMediaCapabilities() {
  ...
  const getCodecWebM = (stream: MediaStream) => {
    ...
    const cleanProfile = stream.Profile.includes('Profile')
      ? stream.Profile.substring(stream.Profile.length - 1)
      : stream.Profile;
   ...
   */
  const getVideoConfiguration = (
    stream: BaseMediaStream,
   ...
   * @returns
   */
  const testVideoAudioStream = async (
    video: BaseMediaStream,
    audio: BaseMediaStream,
  ) => {
    ...
    if (container) {
      config = {
        type: 'media-source',
        video: {
          // avc1.XXXXX dot notation of video part breaks parsing
